...
const fetchTwitterData = async (productName: string) => {
  ...
  try {
    ...
    const data = await response.json() as any; // explicitly cast to any
...
const fetchRedditData = async (productName: string) => {
  ...
  try {
    ...
    const tokenJson = await tokenRes.json() as any; // cast to any
    ...
    const data = await response.json() as any; // cast to any
...
const analyzeSocialTrendsFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    const allData = [...twitterData, ...redditData];
    let totalMentions = 0;
